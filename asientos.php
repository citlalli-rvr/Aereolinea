...
try {
    ...
    // ✅ Obtener todos los asientos de la plantilla
...
<body>
  <div class="container mt-4">
    ...
    <div class="row">
      <div class="col-md-8">
        <!-- Mapa de asientos (igual que antes) -->
        <div class="plane-layout">
          <?php foreach($seat_rows as $rowNum => $row): ?>
            <div class="seat-row mb-2">
              <div class="row-label">Fila <?= $rowNum ?></div>
              <div class="seats-container">
                <?php foreach($row['seats'] as $col => $status): ?>
                  <?php $seatCode = $rowNum . $col; ?>
                  <div class="seat <?= $status === 'X' ? 'occupied' : 'available' ?>" 
                       data-seat="<?= esc($seatCode) ?>"
  ...
  <script>
    // JavaScript para manejar selección de asientos (similar al anterior)
